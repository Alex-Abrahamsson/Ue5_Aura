...
UAuraAttributeSet::UAuraAttributeSet()
{
	...
	// Dia attris
	TagsToAttributes.Add(GameplayTags.Attributes_Secondary_AttackRating, GetAttackRatingAttribute);
	TagsToAttributes.Add(GameplayTags.Attributes_Secondary_DefenseRating, GetDefenseRatingAttribute);
	TagsToAttributes.Add(GameplayTags.Attributes_Secondary_LifeSteal, GetLifeStealAttribute);
	TagsToAttributes.Add(GameplayTags.Attributes_Secondary_ManaSteal, GetManaStealAttribute);
	// Resistances
...
void UAuraAttributeSet::GetLifetimeReplicatedProps(TArray<class FLifetimeProperty>& OutLifetimeProps) const
{
	...
	/* Dia attris */
	DOREPLIFETIME_CONDITION_NOTIFY(UAuraAttributeSet, AttackRating, COND_None, REPNOTIFY_Always);
	DOREPLIFETIME_CONDITION_NOTIFY(UAuraAttributeSet, DefenseRating, COND_None, REPNOTIFY_Always);
	DOREPLIFETIME_CONDITION_NOTIFY(UAuraAttributeSet, LifeSteal, COND_None, REPNOTIFY_Always);
	DOREPLIFETIME_CONDITION_NOTIFY(UAuraAttributeSet, ManaSteal, COND_None, REPNOTIFY_Always);
...
void UAuraAttributeSet::PostGameplayEffectExecute(const FGameplayEffectModCallbackData& Data)
{
	...
	if (Data.EvaluatedData.Attribute == GetIncomingDamageAttribute())
	{
		...
		if (LocalIncomingDamage > 0.f)
		{
			...
			const bool bMiss = UAuraAbilitySystemLibrary::IsMiss(Props.EffectContextHandle);
			ShowFloatingText(Props, LocalIncomingDamage, bBlock, bCrit, bMiss);
		}
	}
...
void UAuraAttributeSet::ShowFloatingText(const FEffectProperties& Props, float Damage, bool bBlocked, bool bCriticalHit, bool bMiss) const
{
	if (Props.SourceCharacter != Props.TargetCharacter)
	{
		if (AAuraPlayerController* PC = Cast<AAuraPlayerController>(Props.SourceCharacter->Controller))
		{
			PC->ShowDmgNumber(Damage, Props.TargetCharacter, bBlocked, bCriticalHit, bMiss);
			return;
		}
...
void UAuraAttributeSet::OnRep_AttackRating(const FGameplayAttributeData& OldAttackRating) const
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UAuraAttributeSet, AttackRating, OldAttackRating);
}

void UAuraAttributeSet::OnRep_DefenseRating(const FGameplayAttributeData& OldDefenseRating) const
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UAuraAttributeSet, DefenseRating, OldDefenseRating);
}
...
void UAuraAttributeSet::OnRep_ManaSteal(const FGameplayAttributeData& OldManaSteal) const
{
	GAMEPLAYATTRIBUTE_REPNOTIFY(UAuraAttributeSet, ManaSteal, OldManaSteal);
}
